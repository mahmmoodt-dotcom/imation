...
export const Navbar: React.FC<NavbarProps> = ({ onPageChange, currentPage }) => {
  const { lang, cart, isLoggedIn, theme, toggleTheme, settings } = useApp();
  ...
  const NavButtons = ({ isMobile = false }: { isMobile?: boolean }) => (
    <>
      ...
      <button 
        ...
        className={`${isMobile ? 'text-[11px]' : 'text-sm'} font-bold uppercase tracking-wider ${currentPage === 'shop' ? 'text-brand' : 'text-gray-500 dark:text-gray-400'} hover:text-brand transition-colors whitespace-nowrap`}
      >
        {t.shop[lang]}
      </button>
      <button 
  ...
  return (
    <nav className="bg-white dark:bg-gray-900 border-b dark:border-gray-800 sticky top-0 z-50 transition-colors shadow-sm">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button 
              ...
              className="relative p-2.5 text-gray-500 dark:text-gray-400 hover:text-brand transition-colors rounded-xl bg-gray-50 dark:bg-gray-800"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
